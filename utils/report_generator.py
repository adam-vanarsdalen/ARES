...
"""
    ...
    all_vuln_findings = [
        finding
        for bucket in ("critical_findings", "high_findings", "medium_findings", "low_findings")
        for finding in vuln_report.get(bucket, [])
    ]
    apt_groups = attribute_findings_to_apt_groups(all_vuln_findings)
    if apt_groups:
        md += "\n## APT Attribution\n\n"
        for group in sorted(
            apt_groups.values(),
            key=lambda item: (-item["technique_count"], item["group"]),
        ):
            alias = f" ({group['also_known_as']})" if group["also_known_as"] else ""
            md += f"### {group['group']}{alias}\n"
            md += f"- **Nation State:** {group['nation_state']}\n"
            md += (
                f"- **Matched Techniques:** {', '.join(group['matched_techniques'])} "
                f"({group['technique_count']} matches)\n"
            )
            md += f"- **Confidence:** {group['confidence']}\n"
            md += f"- **Typical Targets:** {', '.join(group['sector_targets'])}\n\n"
...
"""
    ...
    if tls_posture:
        ...
        tls_coverage = tls_posture.get("coverage", {})
        if tls_coverage.get("certificate") == "failed" or tls_coverage.get("protocols") == "failed":
            md += (
                "- **Coverage gap:** TLS collection was inconclusive. Errors do not demonstrate that protocols "
                "are disabled or that the endpoint has no TLS weaknesses.\n"
            )
...
---
*Generated by ARES — Authorized Recon & Evidence System*
...
"""
    ...
    sigma_rules = build_sigma_rules(target, vuln_report)
    sigma_dir = os.path.join(output_dir, "sigma")
    os.makedirs(sigma_dir, exist_ok=True)
    for rule in sigma_rules:
        safe_title = rule["title"].lower().replace(" ", "_")[:50]
        sigma_path = os.path.join(sigma_dir, f"{safe_title}.yml")
        with open(sigma_path, "w") as f:
            yaml.dump(rule, f, default_flow_style=False, sort_keys=False)
    sigma_note = f"\n_Sigma detection rules: {len(sigma_rules)} rules written to {sigma_dir}_\n"
    md = md.replace(
        "\n---\n*Generated by ARES",
        f"{sigma_note}\n---\n*Generated by ARES",
        1,
